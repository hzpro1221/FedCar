...
from .fedavg_ga_client import FedAvg_GA_Client
from .segformer_b0_avg_ga import SegFormerB0_Avg_GA

class FedAvg_GA_Server:
    def __init__(
        self, 
        num_classes,
        backbone_model, 
        ...
        """
        Initializes the Federated Learning Server with Generalization Adjustment (FedAvg + GA).
        ...
        Args:
            ...
            ga_step_size (float): Step size parameter (d) for Generalization Adjustment.
        """
        print("\n" + "="*50)
        print("[Server] Initializing FedAvg + GA Server...")
        ...
        for i, domain in enumerate(self.source_domains):
            self.clients.append(
                FedAvg_GA_Client.remote(
                    data=domain,
                    client_id=i,
                    local_model=SegFormerB0_Avg_GA(
    ...
    def aggregate(self, local_weights_list, gaps_list, round_idx):
        """
        Generalization Adjustment (GA) aggregation.
        """
        print(f"[Server] Generalization Gaps: {[f'{g:.4f}' for g in gaps_list]}")
        
        mu = sum(gaps_list) / self.num_clients
        max_diff = max([g - mu for g in gaps_list]) + 1e-8 
        
        # d_r = (1 - r/R) * d
        d_r = (1.0 - round_idx / self.num_rounds) * self.ga_step_size
        
        a_temp = []
        for i in range(self.num_clients):
            new_w = self.client_weights[i] + d_r * ((gaps_list[i] - mu) / max_diff)
            a_temp.append(max(new_w, 0.0)) 
    ...
    def evaluate(self, target_domain, checkpoint_path):
        ...
        with torch.no_grad():
            for images, masks in tqdm(self.test_dataloader, desc="Evaluating"):
                ...
                # Only calculate on pixel that is not 255 
