...
class XON_LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        orig_shape = x.shape
        # Handle input 3D (B, N, C) or 4D (B, H, W, C)
        if len(orig_shape) == 4:
            B, H, W, C = orig_shape
            x_in = x.view(B, H * W, C).transpose(1, 2) # Transform to (B, C, N)
        else:
            x_in = x.transpose(1, 2) # (B, C, N)

        out_in = self.instancenorm(x_in)

        if len(orig_shape) == 4:
            out_in = out_in.transpose(1, 2).view(B, H, W, C)
        else:
