...
from .gperxan_client import gPerXAN_Client, replace_bn_with_xan

class gPerXAN_Server(FedAvg_Server):
    def __init__(self, reg_weight, **kwargs):
        self.reg_weight = reg_weight
        super().__init__(**kwargs)
        print(f"[Server] Hyperparams: reg_weight={self.reg_weight}")

    def _build_model(self):
        if self.model_name == 'segformer_b0_gperxan':
            from .segformer_b0_gperxan import SegFormerB0_gPerXAN
            model = SegFormerB0_gPerXAN(num_classes=self.num_classes)
        else:
            model = super()._build_model()
            
        replace_bn_with_xan(model)
        return model

    def _init_worker_pool(self, **kwargs):
        print(f"[Server] Initializing {self.max_concurrent_clients} gPerXAN workers via Ray...")
        workers = []
    ...
    def warm_up_norm_layers(self, dataloader, warmup_steps=50):
        print(f"[Server] Warming up Batch Norm (XAN) statistics for {warmup_steps} steps...")
    ...
    def evaluate(self, target_domain, checkpoint_path=None, apply_bn_warmup=True):
        ...
        if checkpoint_path is not None:
            self.backbone_model.load_state_dict(torch.load(checkpoint_path, map_location=self.device), strict=False)
